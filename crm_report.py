    ...
    def generate_full_report(self, days: int = 30):
        ...
        if leads:
            self.leads_manager.export_to_excel(
                leads, f"{config.REPORTS_DIR}/report_leads_{timestamp}.xlsx"
            )

        if deals:
            self.deals_manager.export_to_excel(
                deals, f"{config.REPORTS_DIR}/report_deals_{timestamp}.xlsx"
            )

        if contacts:
            self.contacts_manager.export_to_excel(
                contacts, f"{config.REPORTS_DIR}/report_contacts_{timestamp}.xlsx"
            )
