...
from pipelines.factories import create_bitnewton_asr, create_bitrix_api, create_vibecode_client, validate_asr_token
...
async def run_sync(args: Any) -> Tuple[int, Path, Path]:
    ...
    try:
        run_result = await process_deals(ctx=processing_ctx, deal_ids=deal_ids, retry_scope=retry_scope)
