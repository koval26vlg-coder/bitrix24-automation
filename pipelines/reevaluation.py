...
async def reevaluate_report(
    args: Any, kpi: dict[str, Any], kpi_cmp: dict[str, Any] | None
) -> tuple[int, Path, Path]:
    ...
    manager_summary_cmp = (
        build_manager_summary(recalculated, score_key="overall_score_cmp")
    ...
    json_out.write_text(
        json.dumps(prepare_report_rows(recalculated), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    xlsx_out = flatten_results(
        recalculated, manager_summary, manager_summary_cmp=manager_summary_cmp
    )
