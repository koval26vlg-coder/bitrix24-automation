...
def flatten_results(
    ...
    with pd.ExcelWriter(out, engine="openpyxl") as writer:
        ...
        _ru_df(deal_df[[c for c in deal_cols if c in deal_df.columns]]).to_excel(
            writer, sheet_name="Сделки", index=False
        )
        if not df.empty:
            call_df = pd.DataFrame(call_detail_rows)
            _ru_df(call_df[[c for c in call_detail_cols if c in call_df.columns]]).to_excel(
                writer, sheet_name="Звонки внутри сделок", index=False
            )
            ...
            conversation_map_df = pd.DataFrame(
                conversation_map_rows, columns=CONVERSATION_MAP_COLUMNS
            )
            ...
            _ru_df(emotional_risk_df).to_excel(
                writer, sheet_name="Эмоциональные риски", index=False
            )
            ci_conversion_factor_df = pd.DataFrame(
                ci_conversion_factor_rows, columns=CONVERSION_FACTOR_COLUMNS
            )
            _ru_df(ci_conversion_factor_df).to_excel(
                writer, sheet_name="Факторы конверсии", index=False
            )
            ...
            _ru_df(script_profile_df).to_excel(
                writer, sheet_name="Соответствие скриптам", index=False
            )
            ...
            sales_stage_score_df = pd.DataFrame(
                sales_stage_score_rows, columns=sales_stage_score_cols
            )
            ...
            manager_criterion_gap_df = pd.DataFrame(
                manager_criterion_gap_rows, columns=manager_criterion_gap_cols
            )
            _ru_df(manager_criterion_gap_df).to_excel(
                writer, sheet_name="Проблемные критерии", index=False
            )
        ...
        if lost_deal_rows or lost_reason_summary_rows or conversion_action_rows:
            ...
            lost_summary_df = pd.DataFrame(
                lost_reason_summary_rows, columns=lost_reason_summary_cols
            )
        ...
        _ru_df(manager_df[[c for c in manager_cols if c in manager_df.columns]]).to_excel(
            writer, sheet_name="Сводка менеджеров", index=False
        )
        if manager_summary_cmp is not None:
            manager_cmp_df = pd.DataFrame(manager_summary_cmp)
            _ru_df(
                manager_cmp_df[[c for c in manager_cols if c in manager_cmp_df.columns]]
            ).to_excel(writer, sheet_name="Сводка менеджеров cmp", index=False)
