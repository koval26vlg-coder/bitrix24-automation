...
def resolve_retry_scope(args: Any) -> Optional[Dict[str, Any]]:
...
def resolve_deal_scope(args: Any, api: Bitrix24API, vibe: Any = None) -> Tuple[Optional[Dict[str, Any]], List[str]]:
    """Shim for tests."""
    retry_scope = resolve_retry_scope(args)
    if retry_scope is not None:
        return retry_scope, list(retry_scope.get("deal_ids", []))
    
    # In async version resolve_deal_ids is async, so this shim can only return empty list if not in retry
