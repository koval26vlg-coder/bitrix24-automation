...
from pipelines.scoring.utils import _context, _first_match, _manager_lines
...
CALL_CHECKLIST_BLOCKS: list[dict[str, Any]] = [
        ...
        "items": [
            {
                ...
                "patterns": [
                    r"\b(добрый|здравств\w*|привет)\b",
                    r"\b(меня зовут|это\s+[а-яё]+|компани[яи])\b",
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(удобно|можете говорить|есть\s+\w*\s*минут|уделить\s+\w*\s*минут|сможете\s+поговорить)\b"  # noqa: E501
                ],
            ...
            },
            {
                ...
                "patterns": [
                    r"\b(как\s+.*к\s+вам\s+обращаться|вас\s+зовут|имя\s+отчество|по\s+имени|[а-яё]{3,}\s+[а-яё]{5,}(?:вич|вна))\b"
                ],
                "partial_patterns": [
                    r"\b(наталья|дмитрий|павел|сергей|андрей|александр|елена|ольга|ирина|мария|клиент)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(звоню\s+по|по\s+поводу|цель\s+звонка|хотел[аи]?\s+обсудить|по\s+вашей\s+заявк\w*|вы\s+оставляли\s+заявку|интересовал[аи]сь)\b"
                ],
            },
        ],
    },
    {
        ...
        "items": [
            ...
            {
                "code": "needs_sequence",
                "criterion": "Вопросы задаются последовательно, менеджер не перескакивает с темы на тему",  # noqa: E501
            ...
            },
            {
                ...
                "patterns": [
                    r"\b(правильно\s+понимаю|верно\s+понимаю|то\s+есть|если\s+я\s+правильно|резюмир\w*|подытож\w*|вы\s+говорите|получается)\b"
                ],
            ...
            },
            {
                ...
                "patterns": [
                    r"\b(давайте|сначала|уточню|перейдем|предлагаю|после\s+этого|следующим\s+шагом|тогда)\b"
                ],
            },
            {
        ...
        "items": [
            {
                ...
                "patterns": [
                    r"\b(касс\w*|ккт|фискаль\w*|офд|маркировк\w*|эквайринг|битрикс|1с|тариф\w*|интеграц\w*|настройк\w*|обслуживан\w*)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(для\s+вас|вы\s+получите|сможете|позволит|выгод\w*|эконом\w*|удобн\w*|быстр\w*|снизит|решит|под\s+вашу\s+задач\w*)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(аналог\w*|конкурент\w*|в\s+отличие|сравн\w*|лучше|хуже|другой\s+вариант|альтернатив\w*)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(стоимост\w*|срок\w*|услов\w*|нюанс\w*|ограничен\w*|можно|нельзя|зависит|точн\w*|подробн\w*)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(например|пример|кейс|практик\w*|у\s+клиентов|обычно|как\s+правило|часто|в\s+похожей\s+ситуации)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(поддержк\w*|гаранти\w*|сервис\w*|опыт|специалист\w*|команд\w*|работаем|партнер\w*|сопровожд\w*)\b"
                ],
            },
        ],
    },
    ...
    },
    {
        ...
        "items": [
            ...
            },
            {
                ...
                "patterns": [
                    r"\b(следующ\w*|дальше|отправ\w*|вышл\w*|перезвон\w*|созвон\w*|встреч\w*|согласу\w*|подготовлю|проверю)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(сегодня|завтра|послезавтра|понедельник|вторник|сред[ау]|четверг|пятниц\w*|час\w*|минут\w*|до\s+\d|после\s+\d|срок|дата)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(вопрос\w*|сомнен\w*|что-то\s+уточнить|осталось|все\s+понятно|понятно\s+ли)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(до\s+свидан\w*|всего\s+добр\w*|хорошего\s+дня|спасибо|до\s+встречи|до\s+связи)\b"
                ],
            },
        ],
    },
        ...
        "items": [
            {
                ...
                "patterns": [
                    r"\b(для\s+вас|вам\s+удобно|под\s+вашу\s+задач\w*|вам\s+подойдет|исходя\s+из|с\s+учетом|как\s+вам\s+удобнее|ваш[еи]\s+интерес)\b"
                ],
            },
            {
                ...
                "patterns": [
                    r"\b(предлагаю|могу|давайте|подготовлю|отправлю|возьму|сделаю|проверю|уточню|согласую)\b"
                ],
            },
            {
            ...
            },
            {
                ...
                "patterns": [
                    r"\b(по\s+вашей\s+заявк\w*|вы\s+интересовал\w*|вижу|обращени\w*|у\s+вас|ваша\s+задач\w*|заявка|заказ|ранее\s+общал\w*)\b"
                ],
            },
        ],
    },
...
def _score_by_patterns(raw: str, item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "score": 0.0,
        "evidence": "",
        "comment": "В расшифровке нет надежного подтверждения критерия.",
    }


def _score_special_checklist_item(
    raw: str, item: dict[str, Any], ctx: dict[str, Any]
) -> dict[str, Any]:
    ...
    if code == "question_variety":
        ...
        if count >= 5:
            ...
            score = 0.0
        evidence = (
            _context(m_text, questions[0].start(), questions[min(count - 1, 2)].end(), radius=160)
            if questions
            else ""
        )
        return {
            "score": score,
            "evidence": evidence,
            "comment": f"Найдено признаков вопросов: {count}. Полный балл — когда менеджер явно задает серию уточняющих вопросов.",  # noqa: E501
        }
    ...
    if code in {"question_sequence", "needs_before_offer"}:
        ...
        evidence = (
            _context(
                m_text, before_offer[0].start(), before_offer[min(count - 1, 2)].end(), radius=160
            )
            if before_offer
            else ""
        )
        if code == "question_sequence":
            comment = f"До презентации найдено вопросов: {count}. Это показывает последовательность выявления потребности."  # noqa: E501
        else:
            comment = f"До предложения найдено вопросов: {count}. Полный балл — когда предложение идет после выявления задачи клиента."  # noqa: E501
    ...
    if code == "speech_clean":
        ...
        return {
            "score": score,
            "evidence": "",
            "comment": f"Найдено явных речевых паразитов/запинок: {len(fillers)}. Долгие паузы лучше подтверждать аудио-метаданными.",  # noqa: E501
        }
...
def evaluate_call_checklist(text: str) -> dict[str, Any]:
    ...
    details = "; ".join(
        f"{b['sales_stage_block_name']}: {b['sales_stage_score']}/{b['sales_stage_max_score']} ({b['sales_stage_percent']}%)"  # noqa: E501
        for b in block_rows
    )
    scores_by_code = {
        str(x.get("checklist_code")): float(x.get("checklist_score") or 0.0) for x in item_rows
    }
    ...
    return {
        ...
        "has_objection_work": bool(objections)
        and any(
            scores_by_code.get(code, 0.0) > 0
            for code in [
                "objection_calm",
