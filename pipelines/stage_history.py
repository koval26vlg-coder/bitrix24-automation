...
def _stage_history_next_start(
    data: dict[str, Any], current_start: int, items_count: int
) -> int | None:
    result = data.get("result")
    ...
    total = safe_int(
        data.get("total") or (result.get("total") if isinstance(result, dict) else None)
    )
...
async def fetch_stage_history_by_deals(
    ...
    clean_ids = [
        int(d)
        for d in dict.fromkeys(str(deal_id) for deal_id in deal_ids if str(deal_id).isdigit())
    ]
...
def summarize_stage_history(
    ...
    for index, item in enumerate(sorted_items):
        ...
        next_dt = (
            parse_dt(sorted_items[index + 1].get("CREATED_TIME"))
        ...
        if (
            semantic in {"S", "F"}
            or stage_id.endswith(":WON")
            or stage_id.endswith(":LOSE")
            or stage_id in {"WON", "LOSE"}
        ):
            reached_final = True
        normalized.append(
            {
                ...
                "stage_duration_hours": (
                    round(duration_minutes / 60.0, 2) if duration_minutes is not None else None
                ),
    ...
    current_age_minutes = (
        _minutes_since(parse_dt(current.get("stage_history_created_at"))) if current else None
    )
    current_age_status = threshold_status(
        current_age_minutes, stage_warning_minutes, stage_critical_minutes
    )
    ...
    path_names = [
        str(item.get("stage_name") or item.get("stage_id") or "").strip() for item in normalized
    ]
    ...
    if not normalized:
        risk = "Нет истории"
        recommendation = "История перемещения по стадиям не найдена. Проверьте права вебхука и корректность выборки."  # noqa: E501
    elif reached_final:
        risk = "Финал"
        recommendation = "Сделка достигла финальной стадии. Для анализа важно сопоставить итог со звонками и причиной результата."  # noqa: E501
    elif current_age_status == "Тревога":
        risk = "Тревога"
        recommendation = (
            f"Сделка находится на текущей стадии дольше критического порога "
            f"({_format_minutes_for_threshold(stage_critical_minutes)}). Нужен следующий шаг или актуализация стадии."  # noqa: E501
        )
    elif total_work_status == "Тревога":
        risk = "Тревога по сроку сделки"
        recommendation = (
            f"Общее время сделки в работе выше критического порога "
            f"({_format_minutes_for_threshold(DEAL_TOTAL_WORK_CRITICAL_MINUTES)}). Проверьте причину задержки по воронке."  # noqa: E501
        )
        ...
        risk = "Возвраты"
        recommendation = "Есть возвраты на предыдущие стадии. Проверьте, не переводится ли сделка вперед без готовности клиента."  # noqa: E501
    elif len(normalized) <= 1:
        risk = "Нет продвижения"
        recommendation = "Сделка пока не двигалась по воронке. Проверьте, есть ли звонок, выявленная потребность и запланированный следующий шаг."  # noqa: E501
    else:
        risk = "OK"
        recommendation = "Движение по стадиям выглядит рабочим. Сопоставьте переходы с качеством звонков и фиксацией следующего шага."  # noqa: E501
    ...
    return {
        ...
        "stage_current_age_days": (
            round(current_age_minutes / 1440.0, 2) if current_age_minutes is not None else None
        ),
        ...
        "deal_total_work_days": (
            round(total_work_minutes / 1440.0, 2) if total_work_minutes is not None else None
        ),
        "deal_total_work_warning_threshold": _format_minutes_for_threshold(
            DEAL_TOTAL_WORK_WARNING_MINUTES
        ),
        "deal_total_work_critical_threshold": _format_minutes_for_threshold(
            DEAL_TOTAL_WORK_CRITICAL_MINUTES
        ),
...
def attach_stage_history_metrics(
    ...
    for row in rows:
        ...
        if not deal_id:
            continue
        row.update(
            summarize_stage_history(
                stage_history_by_deal.get(str(deal_id), []), stage_map=stage_map
            )
        )
